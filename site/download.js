...
async function incrementFirebaseCount(path) {
    try {
        await fetch(`${DB_CONFIG.url}/${path}.json`, {
            method: 'PATCH',
...
async function updateOverview() {
    ...
    const loadBackend = async () => {
        try {
            ...
            setHeaderStatus('API offline', false);
        }
    };
    loadBackend();
    ...
    const winUrl = release?.assets?.find(a => a.name.endsWith('.exe'))?.browser_download_url || 'https://github.com/nightcordoff/nightcordclient-releases/releases/latest';
    const linUrl = release?.assets?.find(a => a.name.endsWith('.AppImage'))?.browser_download_url || 'https://github.com/nightcordoff/nightcordclient-releases/releases/latest';
    const linuxCount = 0;
    ...
    document.getElementById('download-grid').innerHTML = `
        <article class="download-card glass" data-channel="stable">
            ...
            <small id="stable-count">Downloads: ${new Intl.NumberFormat('en-US').format(stableCount)}</small>
            <div class="download-card-spacer"></div>
            <a class="button button-primary" href="${winUrl}" target="_blank" rel="noopener noreferrer" data-action="stable">
...
updateOverview().catch((error) => {
    console.error(error);
    setHeaderStatus('Internal error', false);
});